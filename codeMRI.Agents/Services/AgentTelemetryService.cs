...
public class AgentTelemetryService : IAgentTelemetryService
{
    ...
    public void TrackTaskLifecycle(string taskId, string agentRole, string eventType, bool success,
    ...
    private void InitializeSubscriptions()
    {
        ...
        _messageBus.Subscribe(AgentMessageTypes.All, async message => { await LogMessageAsync(message); });
    }
        ...
        try
        {
            switch (message.MessageType)
            {
                ...
                case AgentMessageTypes.TaskDelegated:
                    ...
                    var delegatedJson = JsonSerializer.Serialize(message.Content);
                    var delegatedData = JsonSerializer.Deserialize<DelegationTelemetryData>(delegatedJson);
                    if (delegatedData != null)
                        TrackDelegation(delegatedData.FromAgent, delegatedData.ToAgent, delegatedData.Reason,
                            delegatedData.TaskId);
                    break;

                case AgentMessageTypes.TaskStarted:
                case AgentMessageTypes.TaskCompleted:
                case AgentMessageTypes.TaskFailed:
                    var lifecycleJson = JsonSerializer.Serialize(message.Content);
                    var lifecycleData = JsonSerializer.Deserialize<LifecycleTelemetryData>(lifecycleJson);
                    if (lifecycleData != null)
                    {
                        var success = message.MessageType != AgentMessageTypes.TaskFailed;
