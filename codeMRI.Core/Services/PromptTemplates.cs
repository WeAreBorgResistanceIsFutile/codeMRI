...
public static class PromptTemplates
{
    ...
    /// </summary>
    public static string ChunkedFindingsPrompt(int chunkIndex, int totalChunks, string previousFindings,
        string currentChunk)
    {
        return $"""
...
";
        ...
        string? humanContext = null)
    {
        ...
        if (!string.IsNullOrEmpty(humanContext))
        {
            ...
            sb.AppendLine(
                "The following documentation was written by humans. Use it to understand the high-level purpose and conceptual details. Prioritize this context over inferred details.");
        ...
        string? humanContext = null)
    {
        ...
        sb.AppendLine(
            $"Your goal is to write a COMPLETE documentation suite for the module '{module.Name}' that serves all stakeholders.");
        sb.AppendLine();
        ...
        sb.AppendLine(
            $"- Architectural Pattern: {module.Metadata.GetValueOrDefault("ArchitecturalPattern", "Not identified")}");
        sb.AppendLine(
            $"- Metrics: Cohesion ({module.QualityMetrics.Cohesion:F2}), Coupling ({module.QualityMetrics.Coupling:F2}), Complexity ({module.ComplexityScore:F1})");
        ...
        sb.AppendLine("- External dependencies (DBs, APIs, Queues) based on: " +
                      (dependenciesList.Any() ? string.Join(", ", dependenciesList) : "None detected") + ".");
    ...
    /// <summary>
    ///     Generates a synthesis prompt for Chain-of-Key output.
    ///     Instructs the LLM to output a structured JSON representing the module's architecture.
    /// </summary>
    public static string ChainOfKeySynthesisPrompt(
        ModuleNode parentModule,
        List<WikiPage> childPages,
        ExtractedEntities entities,
        int crossModuleDependencies,
        string language)
    {
        var childSummaries = childPages
            .Select(p => new
            {
                p.Title,
                Summary = ExtractFirstParagraph(p.Content),
                KeyIdentifiers = ExtractKeyTopics(p.Content).Take(5).ToList()
            });
        ...
        return $$"""
                 You are an expert system architect analyzing code structures.
                 Your task is to synthesize an architectural overview for "{{parentModule.Name}}" (Level {{parentModule.Level}}).

                 ## INPUT: Child Module State
                 {{childSummariesJson}}
                 ...
                 - Cohesion: {{parentModule.QualityMetrics.Cohesion:F2}}
                 - Coupling: {{parentModule.QualityMetrics.Coupling:F2}}
                 - Cross-Module Dependencies: {{crossModuleDependencies}}

                 ## INSTRUCTIONS
                 Output a JSON object that captures the 'State' of this module. This JSON will be used by subsequent synthesis steps.

                 The JSON MUST follow this structure:
                 {
                     "moduleName": "{{parentModule.Name}}",
                     "corePurpose": "High-level purpose of this cluster",
                     "keyProtocols": ["List of main interaction protocols or patterns found in children"],
                     "dataFlow": "Description of how data moves through child modules",
                     "entities": {
                         "classes": {{JsonSerializer.Serialize(entities.ClassNames.Take(10))}},
                         "patterns": {{JsonSerializer.Serialize(entities.PatternNames.Take(5))}}
                     },
                     "narrativeMarkdown": "The full # {{parentModule.Name}} documentation content following our standard overview format, including Mermaid diagrams."
                 }

                 STRICT RULES:
                 1. The 'narrativeMarkdown' field must contain the COMPLETE documentation page.
                 2. Reference children using [[PageTitle]] syntax.
                 3. Use {{language}} for all text content.
                 4. Ensure the JSON is valid and escaped correctly.
