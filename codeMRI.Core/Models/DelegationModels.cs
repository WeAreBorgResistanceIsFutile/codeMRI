...
public class DelegationDecision
{
    ...
    public string Description => Reason switch
    {
        ...
        DelegationReason.TokenLimitExceeded =>
            $"Module exceeds token limit ({Metrics.GetValueOrDefault("TokenCount", 0)} tokens)",
        DelegationReason.ComplexityThresholdExceeded =>
            $"Module complexity too high ({Metrics.GetValueOrDefault("ComplexityScore", 0)})",
        DelegationReason.SemanticDiversityHigh =>
            $"Module has high semantic diversity ({Metrics.GetValueOrDefault("SemanticDiversity", 0):P0})",
        DelegationReason.MaxDepthReached =>
            $"Maximum delegation depth reached ({Metrics.GetValueOrDefault("Depth", 0)})",
        _ => "Unknown delegation reason"
    };

    public static DelegationDecision NoDelegation()
    {
        return new DelegationDecision { ShouldDelegate = false, Reason = DelegationReason.None };
    }

    public static DelegationDecision ForTokenLimit(double tokenCount, int maxTokens)
    {
    ...
    public static DelegationDecision ForComplexity(int complexityScore, int maxComplexity)
    {
        return new DelegationDecision
        {
            ShouldDelegate = true,
    ...
    public static DelegationDecision ForSemanticDiversity(double diversity, double threshold)
    {
        return new DelegationDecision
        {
            ShouldDelegate = true,
    ...
    public static DelegationDecision MaxDepth(int depth)
    {
        return new DelegationDecision
        {
