...
public class DocumentationSynthesisServiceTests
{
    ...
    public async Task SynthesizeParentPageAsync_ShouldPerformMultiStageSynthesis()
    {
        ...
        _mockLlmClient.Verify(x => x.ChatAsync(
            ...
            It.IsAny<List<ChatMessage>>(), It.IsAny<string?>(), It.IsAny<CancellationToken>()), Times.AtLeastOnce, "Should request theme analysis");

        _mockLlmClient.Verify(x => x.ChatAsync(
            It.IsAny<string>(),
            It.Is<string>(s => s.Contains("overview") || s.Contains("synthesis")),
            It.IsAny<List<ChatMessage>>(), It.IsAny<string?>(), It.IsAny<CancellationToken>()), Times.AtLeastOnce, "Should request overview synthesis");
    }
