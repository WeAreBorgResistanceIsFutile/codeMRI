...
public class JudgeAgentServiceTests
{
    ...
    public void SetUp()
    {
        ...
        _judgeAgentService = new JudgeAgentService(_mockLogger.Object, _mockLLMFacade.Object, options);
        ...
        _testPage = new WikiPage
        {
            ...
            Content =
                "This is a test component with comprehensive documentation including examples and API references.",
    ...
    public async Task EvaluateRequirementAsync_WithPositiveResponse_ReturnsScoreOne()
    {
        ...
        _mockLLMFacade.Setup(x => x.ExecuteAsync(It.IsAny<string>(), It.IsAny<string>(), It.IsAny<List<ChatMessage>>(),
            ...
            .ReturnsAsync(new LLMResponse { Content = mockResponse, StrategyUsed = "Simple" });
    ...
    public async Task EvaluateRequirementAsync_WithAmbiguousResponse_ParsesConfidenceScore()
    {
        // Arrange
        var mockResponse =
            "Partially yes. The documentation has some clear sections but overall lacks consistency. I would say 60% meets the requirement.";
        _mockLLMFacade.Setup(x => x.ExecuteAsync(It.IsAny<string>(), It.IsAny<string>(), It.IsAny<List<ChatMessage>>(),
                It.IsAny<MessageCompositionOptions?>(), It.IsAny<CancellationToken>()))
            .ReturnsAsync(new LLMResponse { Content = mockResponse, StrategyUsed = "Simple" });
    ...
    public async Task EvaluateRequirementAsync_WithPercentageInResponse_ExtractsPercentage()
    {
        // Arrange
        var mockResponse =
            "The documentation meets about 75% of the requirement. Some sections are clear but others need improvement.";
        _mockLLMFacade.Setup(x => x.ExecuteAsync(It.IsAny<string>(), It.IsAny<string>(), It.IsAny<List<ChatMessage>>(),
                It.IsAny<MessageCompositionOptions?>(), It.IsAny<CancellationToken>()))
            .ReturnsAsync(new LLMResponse { Content = mockResponse, StrategyUsed = "Simple" });
    ...
    public async Task EvaluateRequirementAsync_WithEmptyResponse_ReturnsDefaultScore()
    {
        ...
        _mockLLMFacade.Setup(x => x.ExecuteAsync(It.IsAny<string>(), It.IsAny<string>(), It.IsAny<List<ChatMessage>>(),
                It.IsAny<MessageCompositionOptions?>(), It.IsAny<CancellationToken>()))
            .ReturnsAsync(new LLMResponse { Content = string.Empty, StrategyUsed = "Simple" });
    ...
    {
        // Act & Assert
        Assert.ThrowsAsync<ArgumentNullException>(() => _judgeAgentService.EvaluateRequirementAsync(_testPage, null!));
    }
    ...
    public async Task EvaluateRequirementAsync_WithMultipleScores_UsesFirstValidScore()
    {
        // Arrange
        var mockResponse =
            "The documentation scores 0.7, but some sections are 0.8 and others are 0.6. Overall it's good.";
        _mockLLMFacade.Setup(x => x.ExecuteAsync(It.IsAny<string>(), It.IsAny<string>(), It.IsAny<List<ChatMessage>>(),
                It.IsAny<MessageCompositionOptions?>(), It.IsAny<CancellationToken>()))
            .ReturnsAsync(new LLMResponse { Content = mockResponse, StrategyUsed = "Simple" });
    ...
    public async Task EvaluateRequirementAsync_WithVariousYesNoResponses_ReturnsCorrectScore(string response,
    ...
    public async Task EvaluateRequirementAsync_WithVariousScoreFormats_ExtractsCorrectScore(string response,
        double expectedScore)
    {
        // Arrange
        _mockLLMFacade.Setup(x => x.ExecuteAsync(It.IsAny<string>(), It.IsAny<string>(), It.IsAny<List<ChatMessage>>(),
