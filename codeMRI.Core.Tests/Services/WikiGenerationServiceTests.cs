...
namespace codeMRI.Core.Tests.Services
{
    ...
    public class WikiGenerationServiceTests
    {
        ...
        public async Task GeneratePageAsync_ShouldIncludeDiagrams_WhenGraphHasNodes()
        {
            ...
            var mockEmbedding = new float[] { 1.0f, 2.0f, 3.0f };
            var mockDocs = new List<Document>
            {
                new Document { FilePath = "Controllers/TestController.cs", Content = "public class TestController { }" }
            };
            ...
            _mockGraphService.Setup(x => x.BuildGraphAsync(It.IsAny<List<CodeComponent>>(), It.IsAny<CancellationToken>()))
                            .ReturnsAsync(graph);
            _mockDiagramGenerator.Setup(x => x.GenerateSequenceDiagramAsync(It.IsAny<EnhancedDependencyGraph>(), It.IsAny<string>()))
                                .ReturnsAsync("```mermaid\nsequenceDiagram\n    TestController->>TestService: Call\n```");
            _mockDiagramGenerator.Setup(x => x.GenerateComponentDiagramAsync(It.IsAny<EnhancedDependencyGraph>(), It.IsAny<string>()))
        ...
        public async Task GeneratePageAsync_ShouldNotIncludeDiagrams_WhenGraphIsEmpty()
        {
            ...
            _mockDiagramGenerator.Verify(x => x.GenerateSequenceDiagramAsync(It.IsAny<EnhancedDependencyGraph>(), It.IsAny<string>()), Times.Never);
        ...
        public async Task GenerateParentPageAsync_ShouldIncludeArchitectureDiagram_WhenGraphHasNodes()
        {
            ...
            {
                new WikiPage { Title = "Child Page 1", Content = "Content for child page 1" },
                new WikiPage { Title = "Child Page 2", Content = "Content for child page 2" }
            };
            ...
            _mockGraphService.Setup(x => x.BuildGraphAsync(It.IsAny<List<CodeComponent>>(), It.IsAny<CancellationToken>()))
                            .ReturnsAsync(graph);
            _mockDiagramGenerator.Setup(x => x.GenerateArchitectureDiagramAsync(It.IsAny<ModuleTree>(), It.IsAny<EnhancedDependencyGraph>()))
            ...
            _mockDiagramGenerator.Verify(x => x.GenerateArchitectureDiagramAsync(It.IsAny<ModuleTree>(), graph), Times.Once);
        }
        ...
        public async Task GeneratePageAsync_ShouldHandleDiagramGenerationFailure_Gracefully()
        {
            ...
            _mockGraphService.Setup(x => x.BuildGraphAsync(It.IsAny<List<CodeComponent>>(), It.IsAny<CancellationToken>()))
                            .ReturnsAsync(graph);
            _mockDiagramGenerator.Setup(x => x.GenerateSequenceDiagramAsync(It.IsAny<EnhancedDependencyGraph>(), It.IsAny<string>()))
                                .ThrowsAsync(new System.Exception("Diagram generation failed"));
