...
using codeMRI.Agents.Models; // Added
using codeMRI.Core.Interfaces;
using codeMRI.Core.Models;
using codeMRI.Shared.Models;
using codeMRI.Visualization.Interfaces;
using codeMRI.Visualization.Models; // Added
...
namespace codeMRI.Agents.Tests.Unit
{
    ...
    public class DocumentationGenerationPipelineTests
    {
        ...
        public async Task GenerateDocumentationAsync_ShouldUsePostOrderTraversal_AndSynthesizeParentPages()
        {
            ...
            { 
                new CodeComponent { Id = "ComponentA", Name = "ComponentA" } 
            };

            // Mock Analysis (Step 1)
            _mockCoordinator.Setup(x => x.CoordinateTaskAsync(It.Is<AgentTask>(t => t.Type == "Analyzer"), It.IsAny<CancellationToken>()))
            ...
            _mockDecompositionService.Setup(x => x.DecomposeHierarchicallyAsync(It.IsAny<string>(), It.IsAny<CancellationToken>()))
                .ReturnsAsync(new ModuleTree { Root = rootNode }); // Should probably use logic to return our tree

            _mockVisualSynthesis.Setup(x => x.GenerateArtifactsAsync(It.IsAny<ModuleTree>(), It.IsAny<EnhancedDependencyGraph>()))
            ...
            _mockCoordinator.Setup(x => x.CoordinateTaskAsync(It.Is<AgentTask>(t => t.Type == "Documenter" && ((CodeComponent)t.Payload).Name == "ComponentA"), It.IsAny<CancellationToken>()))
            ...
            _mockCoordinator.Verify(x => x.CoordinateTaskAsync(It.Is<AgentTask>(t => t.Type == "Documenter"), It.IsAny<CancellationToken>()), Times.AtLeastOnce);
